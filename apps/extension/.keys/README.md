...
(`chrome-extension://<id>` in `trustedOrigins`, see `apps/web/src/worker/auth.ts`) in both
...
```bash
openssl genrsa > apps/extension/.keys/extension.pem            # private key (keep secret)

# manifest "key" value (public key, base64 DER):
openssl rsa -in apps/extension/.keys/extension.pem -pubout -outform DER | base64 | tr -d '\n'

# derive the extension ID:
openssl rsa -in apps/extension/.keys/extension.pem -pubout -outform DER \
...
entry in `apps/web/src/worker/auth.ts` `trustedOrigins`.
