...
The extension is its own workspace (`@timetracker/extension`), built separately from the web app. From the repo root:

```bash
pnpm build:ext        # → apps/extension/dist/
```
...
- `pnpm build:ext` — build to `apps/extension/dist/` (or `pnpm build` inside `apps/extension`).
- `pnpm zip:ext` — build + package `apps/extension/timetracker-extension.zip` for upload.
