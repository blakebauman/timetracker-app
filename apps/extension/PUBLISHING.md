...
This is the end-to-end guide for shipping `apps/extension/` to the Chrome Web Store
...
2. **Host a privacy policy.** Because this extension handles authentication data
   ...
   Use `apps/extension/PRIVACY.md` as the starting draft, host it at a public URL
...
This runs the extension build and writes **`apps/extension/timetracker-extension.zip`** with the
...
`icons/` — never the signing key (`apps/extension/.keys/` is outside `dist/`).

Before zipping, bump `"version"` in `apps/extension/manifest.json` for every new
...
- **Local dev key** (`apps/extension/.keys/extension.pem`) → ID
...
The server (`apps/web/src/worker/auth.ts` → `trustedOrigins`) only trusts the origins
...
3. **Recommended (single ID everywhere):** copy that key (the base64 between the
   ...
   ```
   Put `chrome-extension://<that-id>` in `apps/web/src/worker/auth.ts` `trustedOrigins`,
...
1. **Add new item** → upload `apps/extension/timetracker-extension.zip`.
2. **Store listing** tab:
...
3. **Privacy** tab:
   ...
   - **Permission justifications:**
     ...
     - `host_permissions` (`timetracker.run`, `localhost`) —
...
- `apps/extension/README.md` — architecture, local dev, load-unpacked.
- `apps/extension/SECURITY_AUDIT.md` — security model and hardening.
- `apps/extension/.keys/README.md` — signing key handling.
