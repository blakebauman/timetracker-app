...
- **Keep running:** every successful stop ends on a toast with a five-second "Keep running" that reopens the entry via `PUT /:id { stop: null }` (which nulls `duration` and returns 409 if another entry is already running). A stop that fails for real restores the running timer from a pre-stop snapshot rather than leaving an idle bar over a server entry that is still running.
