...
2. **Chat — ChatAgent DO** over `/agents/*` WebSocket. The frontend (`components/assistant/AssistantPanel.tsx`) uses `useAgent({ agent: "chat-agent" })` + `useAgentChat`. Tools (`lib/assistant-tools.ts`): `startTimer`†, `stopTimer`*, `logTimeEntry`*, `trackMeeting`*, `deleteEntry`*, `getTimeSummary`, `listProjects`, `rememberPreference`*, `searchMemory` — asterisked tools require **human approval** (AI SDK `needsApproval`, surfaced as in-chat confirm cards); `startTimer`† asks only when a timer is already running, because that is the case where it truncates billable time already on the sheet. Tool writes reuse the same D1 helpers + `broadcast()` as REST. **Prompt-injection posture:** a calendar invite is text anyone can put in front of this model, so every calendar title, entry description, project name and memory fact is passed through `lib/untrusted-text.ts` `promptSafe` (no `<`/`>` survives, one line, bounded) before it reaches the prompt, memory content is sanitized on write as well, and the CURRENT FACTS / REMEMBERED PREFERENCES blocks are fenced with a tag minted per turn — so a value can neither form a tag nor guess the closing one. `/agents/*` only routes `chat-agent/<instance>`; anything else is a 404 rather than an unpinned pass-through.
...
- **Offline:** `lib/idb.ts` + `useOfflineSync` queue mutations in IndexedDB and replay on reconnect; timer state is cached so a refresh offline doesn't lose the running timer. A queued mutation rejects with `ApiError { queued: true }`, which entry mutations treat as "not yet" rather than "failed" — the optimistic value stays on screen instead of rolling back under a failure toast and then silently reappearing when the queue drains. Mutations run with TanStack `networkMode: "always"` (`lib/queryClient.ts`): under the default `"online"`, a write made while the browser reported no connection was paused *in memory* and never reached this queue at all. Mutation retries skip queued errors, which would otherwise queue the same write twice.
- **The timer offline:** every stop carries its instant — `PATCH /api/time_entries/:id/stop` takes an optional `{ stop }` (clamped to the server's now and, in SQL, to the entry's start; an absent body keeps the old "stop now" for the extension), so a stop replayed from the queue bills the moment it was pressed, not the moment the network returned. A timer *started* offline has no server id: its queued `POST` is edited in place instead (`lib/pendingStart.ts` `amendQueuedStart`) — Stop sets the create's `stop`, Discard removes it, a description/project edit merges into it — and after the queue drains `adoptReplayedTimer` swaps the placeholder for the replayed entry. Mount-restore consults the queue too, so a reload before the drain neither resurrects a timer stopped offline nor forgets one started offline.
- **The restore window:** the running timer is mirrored synchronously to localStorage alongside IndexedDB (`lib/idb.ts` `readTimerMirror`; `useTimerLifecycle` persists whatever the store holds as running, whichever tab or resync put it there), and `timerStore` seeds itself from that mirror at module load, so a reload shows the running bar in its first frame rather than an idle one with a live Start. Until the restore has heard from `/current`, `timerStore.restoring` is true: the disc is busy, restores aren't announced, and any start — the bar, a row's Continue, a favourite — is *held* with its press instant (`deferredStart`) and applied only if nothing turns out to be running. A start in that window used to stop the timer the restore was about to show, because the server treats a new start as "stop the running one". While restoring, the bar also adopts a running entry from any cached `["time-entries"]` range the moment one arrives, since the list request often beats `/current`. A held start that finds a timer running offers the typed text as that timer's description instead of discarding it.
- **Keep running:** every stop — online or offline — opens a ten-second window (`timerStore.lastStopped`) shown as a pill beside the idle disc, on `Alt+Shift+R`, and on the plain receipt toast. A reopen is `PUT /:id { stop: null }`: one UPDATE whose WHERE carries `NOT EXISTS` another running entry (409 when it matches nothing, so two racing reopens can't both land), nulls `duration`, and broadcasts `timer:start` rather than `entries:changed` so idle tabs and the extension badge pick it up. A timer started *and* stopped offline has no server id; its queued create has the stop taken back off instead (`reopenQueuedCreate`). A stop that fails for real restores the running timer from a pre-stop snapshot rather than leaving an idle bar over a server entry that is still running.
...
- **Billable is resolved, not defaulted.** `time_entries.billable` is the only column reports read to compute both billable seconds and invoiced amount (`reports.ts`), and nothing derives it from the project at read time. `CreateTimeEntrySchema.billable` is therefore deliberately `.optional()` rather than `.default(false)` — with a default the server cannot distinguish "the caller did not say" from "the caller said no", and every entry created without an explicit flag (timer bar, extension, AI quick-add, seeds) landed non-billable regardless of the project it was logged against, so a workspace could track a full week on a billable retainer and report zero revenue. `resolveBillable` (`routes/time-entries.ts`) fills the gap from the project's own flag; an explicit `true`/`false` always wins. Client-side the same precedence runs in the timer bar: explicit toggle > project's flag > the device-local `Default billable` preference (`lib/billable.ts`) > false.
...
- **Global chrome:** `AppShell` mounts the navigation rail (`Sidebar`: an 80px icon rail on desktop, a top strip + sheet on phones), the timer (`TimerBar`: one full-width glass strip docked to the bottom edge — idle it composes, running it's the transport bar with the day ribbon; one component, two bodies, one draft/lifecycle, the disc at the right-hand end of both), command palette (⌘K), keyboard shortcuts, Assistant panel + nudge notifier, and `ProductivityManager` (idle detection, reminders, pomodoro). Every route is a `Pane` (`components/layout/Pane.tsx`): a header that floats over a scroll region which fades out beneath it; `<main>` pads its bottom by `--dock-h` so the last row clears the timer.
...
**Request hygiene** (same pass). Bodies are capped before anything reads them: `hono/body-limit` at 1 MiB on `/api/*` and 64 KiB on `/api/auth/*`; `/mcp` and `/agents/*` are answered before Hono, so `lib/body-guard.ts` checks `Content-Length` and counts the stream (256 KiB). Every list a request can send is bounded in `packages/core/src/schemas.ts` — bulk ids and report id filters at `BULK_ENTRY_IDS_MAX` (90, because D1 stops at 100 bound parameters per statement; the client chunks larger selections in `lib/api.ts`), tags, search, saved-report config size, integration credentials, and parsed-timestamp checks on AI/track-event dates. Errors are uniform: `lib/validate.ts` wraps `@hono/zod-validator` so a 400 is `{ error: <first issue's message>, issues: [{ path, message }] }` and never echoes received values; `app.notFound` is JSON; `app.onError` (`lib/http-errors.ts`) logs one structured line keyed by the `cf-ray` request id, returns that id (`X-Request-Id`) with a generic body, and re-applies the security headers and `no-store` that a throw would otherwise skip. The same header set (`applySecurityHeaders`) lands on every `/mcp` and `/agents/*` response, their 401s and 404s included.

**Realtime hygiene** (low-severity sweep). Both WebSocket upgrades — `/api/ws` and `/agents/*` — refuse a handshake whose `Origin` is not in the CORS allow-list (`isAllowedOrigin` in `middleware/cors.ts`); CORS can decorate but not block a cross-site handshake, and before this the only thing standing in the way was the session cookie's `SameSite=Lax` default. The dev proxy swallows upgrade requests, so this is verified on production with the runbook's curl. `TimerRoom` caps a workspace room at 64 sockets (429 past that) and drops activity heartbeats arriving faster than one per 500 ms per socket — the client sends one per 30 s, so anything faster is a loop. The organization plugin carries explicit ceilings (10 workspaces per user, 25 members, 50 invitations, 48 h invitation expiry) instead of library defaults. `DELETE /api/saved-reports/:id` answers 404 when nothing matched rather than a 204 that hides a wrong id.

**Operations.** Backups (`pnpm backup:d1` → off-platform export; D1 Time Travel for the last 30 days), the restore drill, the production check commands and the secrets inventory are in `docs/RUNBOOK.md`. `pnpm check` runs `scripts/assert-no-secrets-in-client.mjs` after the build: the Cloudflare Vite plugin copies the real `.dev.vars` into the worker output directory, and the tripwire fails the build if a dev-vars file, a secret-shaped string or a source map ever appears under `dist/client`. Worker source maps are emitted (`vite.config.ts` `environments.timetracker_app.build.sourcemap`) and uploaded (`upload_source_maps`) so observability shows real stack frames; the client build stays map-free.
