...
- **Live timer** — type a description in the composer at the bottom of the page, pick project/task/tags, and press the red disc or Enter. The composer also shows today's total and, when your last timer stopped a while ago, how long nothing has been tracked; the project chip turns to a dashed amber outline when you've typed a description but not picked a project, since time without a project can't be billed. While a timer runs the composer docks into a bar along the bottom edge: the elapsed readout with the start time beneath it ("since 09:02"), the editable description and pills, a ribbon of today's tracked time, and Stop. If you started late, click the readout to set the elapsed time, or click the start time to type the clock time you actually began. Pressing Enter in the description while a timer runs saves the description — it does not stop the timer; stop with the disc or `Alt+Shift+S`. Tags can be added from the tag chip in the bar while you track. For ten seconds after every Stop — including offline — a **Keep running** button sits beside the Start disc (or press `Alt+Shift+R`): if you stopped by mistake, the same entry picks up from its original start, in every open tab. The plain receipt toast ("Saved 47m to EY Audit") offers it too. Picking a suggestion while a timer runs applies its whole combination; if that moves the timer to a different project, a toast says so with **Undo**. Typing into a running timer's description to search for a suggestion doesn't change the entry until you pick one or leave the field; press Escape to put the saved description back. On a phone the running bar's Stop disc sits at the bottom-right.
