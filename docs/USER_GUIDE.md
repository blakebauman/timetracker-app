...
3. **Start a timer** from the timer bar along the bottom of the page: type a description, pick a project, hit the red start button at its right-hand end.
...
- **Live timer** — type a description in the timer bar along the bottom of the page, pick project/task/tags, and press the red disc at its right-hand end (or Enter). The bar also shows a ribbon of today's tracked time, today's total and, when your last timer stopped a while ago, how long nothing has been tracked; the project chip turns to a dashed amber outline when you've typed a description but not picked a project, since time without a project can't be billed. While a timer runs the same bar becomes the transport: the elapsed readout with the start time beneath it ("since 09:02"), the editable description and pills, a ribbon of today's tracked time, and Stop — in exactly the spot Start was. If you started late, click the readout to set the elapsed time, or click the start time to type the clock time you actually began. Pressing Enter in the description while a timer runs saves the description — it does not stop the timer; stop with the disc or `Alt+Shift+S`. Tags can be added from the tag chip in the bar while you track. For ten seconds after every Stop — including offline — a **Keep running** button appears beside the description field (or press `Alt+Shift+R`): if you stopped by mistake, the same entry picks up from its original start, in every open tab. The plain receipt toast ("Saved 47m to EY Audit") offers it too. Picking a suggestion while a timer runs applies its whole combination; if that moves the timer to a different project, a toast says so with **Undo**. Typing into a running timer's description to search for a suggestion doesn't change the entry until you pick one or leave the field; press Escape to put the saved description back. On a phone the Start and Stop disc sits at the bottom-right of the bar.
...
- **Continue last** — the history-clock button in the idle timer bar picks up whatever you tracked most recently, with its project, task, tags and billable flag. It's there because three of the five Timer views (calendar, timesheet, planner) show no entry rows, so the per-row **Continue** isn't reachable from them. It only appears when there's something to continue.
- **Favorites** — save a description + project + task + billable combo and start it with one click from the star menu in the idle timer bar.
...
- **Starting and stopping work offline too**, and keep their real times. A timer started offline runs normally and syncs with the moment you pressed Start; stopping it (or any timer) offline records the moment you pressed Stop, not the moment the connection came back. While something is waiting to sync, a small cloud icon sits beside the running timer's start time.
- When the connection comes back and queued changes sync, a "Back online — N offline changes synced" notice confirms it. An offline stop of an entry with no project still warns that the time can't be billed.
- If a Stop genuinely fails (the server rejects it), the timer comes back on screen still running, with a **Try again** that retries at the time you originally pressed Stop.
