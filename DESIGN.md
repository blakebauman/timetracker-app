...
Time Tracker is a rack of instruments, not a dashboard. The chassis is a true-neutral ramp with no tint anywhere — a near-black ground in dark, a light grey in light — and the panels sit in it with one hairline edge each. Navigation is an 80px icon rail on the left; every route is a tall pane whose header floats over a scroll-fade and whose content scrolls without a scrollbar. The timer is one glass strip docked to the bottom edge, full width, in both of its states: idle it asks what you're working on, running it is a transport bar — and in both the day's trace sits in the same place and the red disc sits at the right-hand end, so Start and Stop are pressed in the same spot. One saturated colour, the brand red, is the only lit control on the rack: the Start disc and its glow, the elapsed readout, the running segment and now-line, the active rail ring, the header's one primary pill, the Settings underline. Project and tag swatches do the at-a-glance scanning; nothing else on the page carries hue.
...
- Controls are pills, panels are 12px-cornered, the timer strip is square to the edge it docks to; radius is chosen by what a thing *is*.
...
- Motion is one scale (150/200/300ms) and one curve. The timer strip doesn't move between its states: idle and running are the same shape in the same place.
...
- **Brand Red as text** (`--primary-ink`, `{colors.primary-ink-light}` light / `{colors.primary-ink-dark}` dark): the elapsed readout in the docked bar, the billable "$" on a row (the *toggle* in the timer strip is not red — "on" is a recessed pill in full ink, so the bar keeps one lit control), links, the hover colour of an editable description or duration. `--primary` is calibrated as a *fill behind white* and fails AA as small type; this holds hue and chroma and moves lightness only. Light is L 0.485, not 0.5: on the accent surface plus its own 10% tint the older value measured 4.43:1 (`e2e/contrast.spec.ts`).
...
- **Glass** (`--glass`, `{colors.glass-light}` light / `{colors.glass-dark}` dark): the card colour at partial opacity behind a 24px blur with 1.2 saturation. Exactly one surface is made of it — the docked timer strip, idle or running — declared as a token so nothing else drifts into a hand-tuned alpha.
...
**The dock clearance.** The timer strip publishes its rendered height as `--timer-h`; the shell exposes it as `--dock-h` and the main region pads its bottom by it, so the last row of any route clears the strip at every width. Toasts offset 120px from the bottom on desktop for the same reason; on a phone the strip spans the bottom edge, so toasts drop in from the top instead, 72px down to clear the brand strip.

**Density and rhythm.** The base rhythm is 4px; the working steps are 8 (row gap, rail button gap), 12 (header gap, timer strip vertical padding, row card horizontal gap), 16 (row card side padding, phone insets), 24 (pane gutters, card padding, the timer strip's side padding from md). Entry rows are row cards 8px apart under a plain day header; Settings caps its panels at 768px. Breakpoints are Tailwind's: 640 (sm — the row's time range moves into the metadata line below it), 768 (md — the rail appears, the timer strip goes to a single row), 1024 (lg — the day ribbon appears in the bar, Split view is offered), 1280 (xl — the ribbon widens from 256 to 320px).
...
- **The lit disc** (large shadow, coloured `--primary` at 40%, 50% on hover): the idle timer's 40px Start disc and the assistant's Send disc — a glow, not a drop. The only coloured shadows in the system. Running, the Stop disc drops the glow and the pulse ring does that job.
...
**The Glass Rule.** Exactly one surface is glass — the docked timer strip, idle or running — `--glass` behind a 24px blur. Nothing else blurs.
...
### The Idle Timer (signature)
The idle body of the docked timer strip — the same glass strip as the running bar (`--glass`, 24px blur, hairline top, square, full width from the rail's right edge, 16/24px side and 12px vertical padding plus the safe-area inset, capped at 1800px inside), so starting a timer changes what the strip says, not where or what shape it is. Neither body animates in. Left to right: the description column (capped at 42rem, `basis-0`), the day ribbon (from lg, in the running bar's place; its now-line is `--foreground` at 40% while nothing runs), the day summary (today's total in mono, and "· 42m untracked" when the last stop was five or more minutes ago), resume-last and favourites, and at the right-hand end the 40px `--primary` Start disc with its red glow, scaling to 105% on hover — in exactly the position of the running bar's Stop disc. The column's first row is the *bare* description field — no border, transparent, 36px, Body-size at every width, placeholder "What are you working on?", ellipsis on overflow, a 4% ink hover wash, focus as an inset full-opacity ring — then the billable toggle (whether an hour is invoiceable is a property of the work described), then, once something is typed and the field is focused (fine pointer, sm and up), a quiet "Enter to start" kbd hint. For ten seconds after any stop, and only while the field is empty, a neutral outline **Keep running** pill (Alt+Shift+R; icon-only on a phone) sits in the right-hand group beside resume and favourites, next to the disc it undoes — the undo lives in the strip, not only in a toast across the screen. The column's second row is the chips on one line, truncating — project, task, tag pills (32px, hairline, `--background`, hovering to a 6% ink wash; plus a tag picker chip — an icon beside the tag pills, carrying the count below xl where the pills fold away — that adds, lists and removes). On a phone the day summary, resume and favourites take the first row (where the running body has its readout), the field the second, the chips the third, ending in the Start disc bottom-right. An unassigned project chip takes a dashed `--warning` edge, `--warning-ink` and the visible label "Add project" once there's a description to bill, with the reason as its accessible description. The disc is last in the DOM, so Tab follows describe → assign → start. Until the page has heard whether a timer is running, the disc holds a Spinner and is `aria-busy`: a press is held, with its instant, and applied only if nothing turns out to be running (the bar adopts a running entry from the entries list as soon as it arrives). The day summary and the ribbon are Skeletons meanwhile — unknown is never drawn as idle or as zero.

### The Docked Transport Bar (signature)
The running timer: the same docked strip as the idle body. While today's entries load, the full ribbon is a Skeleton track labelled "Loading today", never an empty day; its hour labels fall on the clock's multiples of three. Left to right: the Readout (24px mono 600, `--primary-ink`, in a fixed 8ch box its editor shares so editing never shifts the row, click to edit in place) over the start time ("since 09:02", Label mono, muted, click to type a clock time; a cloud-off glyph beside it while writes are queued), the same bare description field (now at 500, with a 4% ink hover wash so it reads as editable; Enter commits it and never stops the timer) and the same chips, the day ribbon (from lg, flexing between 12 and 28rem beside a description column capped at 42rem, so no band of empty glass opens up at wide widths), a ghost Discard button that hovers to `--destructive`, and at the right-hand end the 40px Stop disc — brand red, shadow off, a flat `--primary` ring breathing outward behind it — where the idle body's Start disc was. The description column takes `basis-0`: it absorbs the leftover width rather than pushing Discard onto a row of its own. Below the tablet breakpoint the readout and Discard share the first row, the field (with the billable toggle) takes the second, and the chips the third on one line, the project chip truncating — ending in the Stop disc, bottom-right, where a right thumb rests; the bottom padding adds the safe-area inset. The bar publishes its rendered height as `--timer-h`, which panes pad by. On every Start and Stop, keyboard focus is carried to the equivalent control in the other body and a polite live region says what happened.
...
- **Do** pick radius by family: pills for controls, 12px for containers, square for the docked timer strip, the base scale for data cells (**Shapes**).
...
- **Don't** put a resting shadow on a card, row, input or button, and don't add hover lift to panels. A shadow means off-the-page: overlays and the lit disc.
- **Don't** add a second glass surface, a floating second timer, or a third form of "running" — extend the pulse ring or the running dot.
...
**Curves.** `--ease-out-quart` (cubic-bezier 0.25, 1, 0.5, 1) is *the* curve; there is no separate enter and exit. There is no second curve: the one large slow move it existed for — the timer surfaces arriving — is gone now that the timer is a single docked strip. Symmetric breathing stays on ease-in-out.
...
**The strip doesn't move.** Idle and running are the same docked strip in the same place, so neither body animates in when it replaces the other (`--animate-dock-in` and `--animate-capsule-in` are retired with the floating composer); what changes is inside — the disc's glyph scales in, the readout fades up.
...
| dock | 30 | The app-wide timer strip, idle or running. Above every pane, below every portal, so a dialog still covers the running readout. |
