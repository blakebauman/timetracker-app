...
`/mcp` is intercepted in `index.ts` **before** Hono (like `/agents/*`), authenticated by an API key rather than a session, and served by `agents/mcp`'s `createLegacyMcpHandler` — Streamable HTTP, stateless, no Durable Object. A fresh `McpServer` is built per request bound to the resolved workspace. Two invariants: **no tool takes a workspace id** (it's fixed at construction, so nothing a model invents reaches a tenant boundary), and **write tools are registered only for a `read_write` key** (a read key isn't shown them at all). Every tool wraps a helper the REST API already uses, so a chat answer and a Reports answer can't disagree.
...
- **Email digests** (`runDigests`): the opt-in morning briefing and Monday weekly summary. The cron has no request to read a timezone from, so it works off `user.digest_tz_offset` (reconciled client-side in `useHydrateSettings` when it drifts, so a DST change doesn't send an hour off for months). Exactly-once per day: the cron ticks twelve times inside the target hour, so each tick **claims** the send first with a conditional `UPDATE` of `digest_daily_sent`/`digest_weekly_sent` to the user's **local** date (only one tick's update lands), sends, and releases the claim on failure so the next tick retries. Marking after the send used to re-mail on a partial failure.
...
- **The timer has two bodies and one mind** (`components/timer/TimerBar.tsx`): both are the same full-width `tt-glass` strip docked to the bottom edge, with the disc (`TransportDisc`) last — far right on desktop, bottom-right on a phone — so Start and Stop are pressed in the same spot. Idle = description/chips, `DayRibbon`, day summary, resume/favourites, red Start disc with a red shadow; running = elapsed readout at `text-2xl` mono in `--primary-ink` (`TimerControl` with `withDisc={false}`), the same description/pills, `DayRibbon` with the live red segment, Discard, Stop disc + pulse ring. Both are `fixed`, `z-dock`, and share one draft/lifecycle. The e2e contract is `header[aria-label="Timer controls"]` and the control aria-labels.
- **The rail** (`components/layout/Sidebar.tsx`): 48px circular buttons, the active one lifted with a brand-red ring that fades downward (`tt-rail-ring`), the brand mark glowing in dark (`tt-brand-glow`), Assistant / search / account at the bottom. Compute active state with `useLocation`, never NavLink's function-form `className` — a Tooltip's `asChild` slot string-joins it and every class is lost. Phones get a top strip + sheet nav.
- **Icon buttons use the `Button` size tokens** (`icon-xs` = 24px, `icon-sm` = 32px, `icon-lg` = 40px), never `size="icon"` with an ad-hoc `h-N w-N` override. Prefer icon-only actions (with `aria-label` + `title`) in dense toolbars; keep the label when the button conveys current state (date range, rounding mode).
- **Layering is a named scale**, not raw numbers: `z-sticky` / `z-overlay` / `z-dock` / `z-portal` / `z-tooltip`. A bare `z-50` fails lint.
- **Radius is chosen by what an element is** (The Geometry Rule): controls are `rounded-full` (buttons — icon-only ones are circles — badges, inputs, segment tracks), containers are `rounded-container` (12px: cards, dialogs, sheets, popovers, and list rows, which are cards), the docked timer strip is square, and dense data cells keep `rounded-md`/`-lg` (timesheet/planner cells, calendar blocks).
- **Cards carry a 1px hairline and no shadow.** `--card` is one step lighter than the ground in both themes and `--border` (one device pixel, one value) is the panel edge; there is no 2px border and no resting shadow anywhere. Inputs and selects are `bg-background` — recessed wells on a card. The only shadows are overlays and the lit Start disc.
