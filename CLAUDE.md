...
- **Radius is chosen by what an element is** (The Geometry Rule): controls are `rounded-full` (buttons — icon-only ones are circles — badges, inputs, segment tracks), containers are `rounded-container` (12px: cards, dialogs, sheets, popovers, and list rows, which are cards), the composer alone is `rounded-capsule` (2rem), and dense data cells keep `rounded-md`/`-lg` (timesheet/planner cells, calendar blocks).
